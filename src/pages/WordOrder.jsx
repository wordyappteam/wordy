...
export default function WordOrder() {
  ...
  if (phase === 'picker') {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-yellow-50 flex flex-col">
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-yellow-50 flex items-center justify-center">
                              ...
                              status === 'loading'
                                ...
                                : status === 'added'
                                  ...
                                  : status === 'error'
                                    ...
                                    : 'bg-gray-50 text-gray-700 border-gray-200 hover:bg-indigo-50 hover:text-indigo-700 hover:border-indigo-200 active:scale-95'
                            }`}
                          >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-yellow-50 flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-8 gap-6">
        ...
        {/* The sentence to build IS the exercise, so it carries the page. It was
            set at text-lg in italics with a caption above it, which read as a
            footnote to the word bank rather than the thing being asked. */}
        <div className="w-full max-w-lg text-center">
          <p className="text-[11px] text-gray-400 uppercase tracking-[0.14em] mb-2.5">
            {lang === 'uk' ? 'Складіть речення' : 'Build the sentence'}
          </p>
          <p className="text-2xl leading-snug font-semibold text-gray-900">{card.english}</p>
          {card.headword && (
            <span className="inline-block mt-3 px-2.5 py-1 rounded-full bg-yellow-50 text-yellow-700 text-[11px] font-semibold tracking-wide">
              {card.headword}
            </span>
          )}
        </div>
        ...
        <div className="w-full max-w-lg">
          {/* No "Your answer" label and no dashed outline: the dashes read as an
              empty form field or a drag target, and this exercise is tap-based.
              A solid card with the words sitting in it reads as a sentence being
              assembled, which is what it is. */}
          <div className="min-h-[76px] bg-white rounded-2xl border border-gray-200 shadow-sm px-4 py-4 flex flex-wrap gap-2 items-center justify-center">
            {placed.length === 0 && (
        ...
        {/* One block, two states. This was two nearly identical green panels with
            a duplicated speak button; a wrong answer being reported in the same
            green as a right one also made the two hard to tell apart at a glance.
            Right = green. Wrong = wheat, showing the order you were after. */}
        {checked && (
          <div className={`w-full max-w-lg rounded-2xl px-4 py-3 border ${isCorrect ? 'bg-green-50 border-green-200' : 'bg-yellow-50 border-yellow-200'}`}>
            {!isCorrect && (
              <p className="text-[11px] text-yellow-700 uppercase tracking-wide text-center mb-1.5">
                {lang === 'uk' ? 'Правильний порядок' : 'Correct order'}
              </p>
            )}
            <div className="flex items-center justify-center gap-3">
              <p className={`font-semibold ${isCorrect ? 'text-green-700 text-sm' : 'text-yellow-900 text-base'}`}>
                {isCorrect ? (lang === 'uk' ? '✓ Правильно!' : '✓ Perfect!') : card.german}
              </p>
              <button
                onClick={handleSpeak}
        ...
        {!checked && (
          <div className="w-full max-w-lg">
            <p className="text-[11px] text-gray-400 uppercase tracking-[0.14em] mb-2.5 text-center">{lang === 'uk' ? 'Слова' : 'Words'}</p>
            <div className="flex flex-wrap gap-2 justify-center">
              {bank.map((chip) => (
                <button
                  key={chip.key}
                  onClick={() => placeWord(chip)}
                  className="px-3.5 py-2 bg-white border border-gray-200 hover:border-indigo-400 hover:bg-indigo-50 rounded-xl text-base font-medium text-gray-700 transition-all hover:scale-105 active:scale-95 shadow-sm"
                >
                  {chip.word}
                </button>
              ))}
          ...
          {!checked ? (
            <>
              ...
              <button
                onClick={handleCheck}
                disabled={!allPlaced}
                className="px-8 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-200 disabled:cursor-not-allowed text-white rounded-2xl text-sm font-semibold transition-all shadow-sm"
              >
            ...
            <button
              onClick={handleNext}
              className="px-10 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl text-sm font-semibold transition-all shadow-sm"
            >
