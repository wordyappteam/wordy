// Parser for the bulk "import list" feature: turns one pasted line into a
// provisional dictionary entry. POS here is a best-effort guess from the text
// format; the AI identify step overwrites it (and fills translations/examples),
// so this only needs to be right often enough to give a useful preview.

const PREPOSITIONS = ['auf','an','für','über','um','von','mit','nach','zu','bei','in','gegen','durch','aus','als']

// German POS heuristic for a bare word with no article or conjugation to key off:
// nouns are ALWAYS capitalized; lowercase infinitives end in -en / -eln / -ern.
// Everything else falls back to adjective/adverb. Deliberately imperfect — a few
// lowercase adjectives/adverbs end in -en (offen, eigen, gern) and will guess
// "verb", but the AI re-identify corrects POS, so approximate is fine.
export function guessBarePos(word) {
  if (/^[A-ZÄÖÜ]/.test(word)) return 'noun'
  if (/(?:en|eln|ern)$/.test(word)) return 'verb'
  return 'adjective'
}
...
export function parseBulkLine(line) {
  ...
  // Bare word (no article, no parens, no preposition pattern): guess POS from
  // German morphology instead of blindly defaulting to adjective.
  const word = line.replace(/,.*/, '').replace(/\(.*\)/, '').trim()
  if (!word) return null
  return { word, form: null, pos: guessBarePos(word), entry_type: 'word', translation: '', status: 'new' }
}
